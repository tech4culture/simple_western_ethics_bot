...
def get_western_ethics_config():
    ...
    return {
        ...
        "Critiquedelaraisonpure.pdf": {
            ...
            "author": "Kant", "language": "français", "type": "pdf"
        },
        ...
        "ethique_de_Spinoza.pdf": {
            ...
            "author": "Spinoza", "language": "français", "type": "pdf"
        },
        ...
        "Gorgias.pdf": {
            ...
            "author": "Platon", "language": "français", "type": "pdf"
        },
...
def load_file_versatile(filepath, skip_start=0, skip_end=0):
    ...
    print(f"📖 Chargement: {filepath.name} (type: {filepath.suffix})")
...
def load_pdf_with_skip_pages(filepath, skip_start=0, skip_end=0):
    ...
    print(f"📄 Pages gardées: {start_page} à {end_page-1} (soit {end_page-start_page} pages)")

    if start_page >= end_page:
        print(f"❌ ERREUR: Skip trop important (start={start_page}, end={end_page})")
        return []
    ...
    print(f"✅ Texte extrait: {len(clean_text)} caractères")
...
class SimpleLangDetector:
    """Détecteur simple - AJOUTEZ AVANT VOTRE BOT"""

    def detect_language(self, text: str) -> str:
        text_lower = text.lower()

        # Compteurs simples
        fr_score = sum(1 for word in ['qu\'est-ce', 'comment', 'éthique', 'justice'] if word in text_lower)
        en_score = sum(1 for word in ['what', 'how', 'ethics', 'justice'] if word in text_lower)
        es_score = sum(1 for word in ['qué', 'cómo', 'ética', 'justicia'] if word in text_lower)
...
class BulletproofRAGSystem:
    """
    Système RAG qui évite les écrasements et permet l'ajout incrémental
    """

    def __init__(self, db_path="./chroma_db_bulletproof", anthropic_api_key=None):
        print("🛡️  Initialisation système RAG Bulletproof")
        ...
        print("🌍 Chargement modèle embedding multilingue...")
    ...
    def get_collection_safely(self, collection_name="western_ethics_safe"):
        """Récupère ou crée une collection de manière sûre"""
        try:
            collection = self.client.get_collection(collection_name)
            print(f"📂 Collection '{collection_name}' chargée: {collection.count()} documents")
        except:
            print(f"📝 Création nouvelle collection '{collection_name}'")
            collection = self.client.create_collection(
                name=collection_name,
                metadata={
                    "description": "Western Ethics - Anti-Écrasement",
    ...
    def analyze_available_sources(self, folder_path="./Livres_philo"):
        """Analyse les sources disponibles vs celles déjà traitées"""
        ...
        for filename, file_config in config.items():
            ...
            if filepath.exists():
                ...
                if filename in self.sources_registry:
                    ...
                    print(f"✅ {filename} - Déjà traité ({file_info['size']} bytes)")
                else:
                    available_sources.append(file_info)
                    print(f"🆕 {filename} - Nouveau ({file_info['size']} bytes)")
            else:
                missing_sources.append(filename)
                print(f"❌ {filename} - Manquant")

        print(f"\n📊 RÉSUMÉ:")
        print(f"   🆕 Nouveaux: {len(available_sources)}")
        print(f"   ✅ Déjà traités: {len(already_processed)}")
        print(f"   ❌ Manquants: {len(missing_sources)}")
    ...
    def process_single_source_safely(self, source_info):
        """Traite UNE source de manière isolée et sûre"""
        filename = source_info["filename"]
        print(f"\n📖 TRAITEMENT ISOLÉ: {filename}")
        print("-" * 40)
        ...
        try:
            ...
            if not documents:
                print(f"❌ Aucun contenu extrait de {filename}")
                return None

            print(f"✅ Texte extrait: {len(documents[0].text)} caractères")

            # Enrichir avec métadonnées SÉCURISÉES
            enhanced_doc = self._add_bulletproof_metadata(documents[0], source_info)

            # Chunking ISOLÉ (pas de mélange possible)
            chunks = self._chunk_single_document(enhanced_doc, source_info)

            print(f"✂️  {len(chunks)} chunks créés pour {filename}")
    ...
    def _add_bulletproof_metadata(self, document, source_info):
        """Ajoute des métadonnées bulletproof"""
        from llama_index.core import Document

        # Métadonnées enrichies et sécurisées
...
{document.text}"""
    ...
    def _chunk_single_document(self, document, source_info):
        ...
        # Vérifier que chaque chunk garde les métadonnées
        verified_chunks = []
        for i, chunk in enumerate(chunks):
            if "[BULLETPROOF_METADATA]" in chunk:
                verified_chunks.append(chunk)
            else:
                # Reconstituer les métadonnées si perdues
    ...
    def _repair_chunk_metadata(self, chunk, source_info, chunk_index):
        """Répare un chunk qui a perdu ses métadonnées"""
...
{chunk}"""
    ...
    def add_sources_incrementally(self, collection_name="western_ethics_safe"):
        """Ajoute les sources de manière incrémentale et sûre"""
        print("🔄 AJOUT INCRÉMENTAL SÉCURISÉ")
        print("=" * 60)
        ...
        for source_info in analysis["new_sources"]:
            ...
            if chunks_added > 0:
                ...
                print(f"✅ {source_info['filename']}: {chunks_added} chunks ajoutés")
            else:
                print(f"❌ {source_info['filename']}: Échec ajout")
        ...
        print(f"\n🎉 AJOUT TERMINÉ")
        print(f"📊 Collection: {initial_count} → {final_count} documents (+{total_chunks_added})")

        return collection

    def _add_chunks_to_collection(self, collection, chunks, source_info, start_index):
        """Ajoute les chunks à la collection de manière sûre"""
        print(f"💾 Ajout de {len(chunks)} chunks pour {source_info['filename']}")
        ...
        try:
            for i in range(0, len(chunks), batch_size):
                ...
                print(f"   💾 {chunks_added}/{len(chunks)} chunks ajoutés")

            return chunks_added

        except Exception as e:
            print(f"❌ Erreur ajout chunks: {e}")
            return 0

    def search_by_author(self, collection, author, query, n_results=3):
        """Recherche filtrée par auteur (évite les mélanges)"""
        print(f"🔍 Recherche '{query}' chez {author}")
        ...
        try:
            ...
            print(f"✅ {len(results['documents'][0])} résultats trouvés pour {author}")
            return results
    ...
    def ask_philosopher(self, collection, author, question, show_sources=True):
        ...
        if not search_results or not search_results['documents'][0]:
            return {
                'answer': f"❌ Aucun contenu trouvé pour {author}",
                'sources': [],
                'question': question,
                'author': author
            }
...
TONE: Scholarly but accessible - maintain philosophical depth while being clear."""
 ...
 ### cette partie a déjà été testée et je pourrais sans doute l'enlever lorsque l'ensemble du programme aura ete testé ###

    def verify_collection_integrity(self, collection):
        """Vérifie l'intégrité de la collection"""
        ...
        for doc, meta in zip(sample["documents"], sample["metadatas"]):
            ...
            # Vérifier bulletproof
            if meta.get("bulletproof"):
                bulletproof_count += 1

        print(f"📊 Auteurs trouvés: {authors_found}")
        print(f"📁 Sources trouvées: {sources_found}")
        print(f"🛡️  Chunks bulletproof: {bulletproof_count}/{len(sample['documents'])}")

        if len(authors_found) > 1:
            print("✅ Collection multi-auteurs détectée")
        else:
            print("⚠️  Collection mono-auteur - vérifiez si normal")
...
# =============================================================================
# INTERFACE SIMPLIFIÉE - à terme les tests bullet proof peuvent sans doute être enlevés
# =============================================================================

def bulletproof_setup(anthropic_key=None):
    """Setup rapide du système bulletproof"""
    print("🚀 SETUP SYSTÈME BULLETPROOF")
    print("=" * 50)
    ...
    print(f"🛡️  Intégrité: {integrity['bulletproof_ratio']:.1%}")

    return system, collection

def test_bulletproof_search(system, collection):
    """Test des recherches séparées par auteur"""
    print("\n🧪 TEST RECHERCHES SÉPARÉES")
    print("=" * 50)
...
class SimpleLangDetector:
    def detect_language(self, text: str) -> str:
        # Détection simple
        if any(word in text.lower() for word in ['qu\'est-ce', 'comment', 'éthique']):
            return 'français'
...
# =============================================================================
# SYSTÈME DE DÉTECTION ÉTHIQUE INTELLIGENT
# =============================================================================

class EthicsDetector:
    """Détection intelligente des questions éthiques avec LLM"""

    def __init__(self, anthropic_client):
        self.anthropic_client = anthropic_client
        self.cache = {}  # Cache pour éviter les reclassifications
...
Answer:"""
            ...
            print(f"⚠️ Erreur classification LLM: {e}")
            # Fallback intelligent avec mots-clés multilingues
            return self._fallback_classification(question)

    def _fallback_classification(self, question: str) -> bool:
        """Fallback avec mots-clés étendus et multilingues"""
        ethics_keywords = {
            'fr': ['éthique', 'moral', 'justice', 'vertu', 'bien', 'mal', 'devoir', 'vivre', 'juste'],
            'en': ['ethics', 'moral', 'justice', 'virtue', 'good', 'evil', 'duty', 'live', 'right', 'fair'],
            'es': ['ética', 'moral', 'justicia', 'virtud', 'bien', 'mal', 'deber', 'vivir', 'justo'],
            'de': ['ethik', 'moral', 'gerechtigkeit', 'tugend', 'gut', 'böse', 'pflicht', 'leben', 'recht']
        }
...
# =============================================================================
# CORRECTIONS FINALES POUR LES PROBLÈMES IDENTIFIÉS
# =============================================================================
...
class ImprovedWesternEthicsBot:
    ...
    def __init__(self, rag_system, collection, anthropic_client):
        ...
        print("✅ Bot final initialisé!")

    def _get_system_prompt_for_language(self, author):
        language_prompts = {
        'français': f"Vous êtes {author}, un philosophe de référence. Répondez ENTIÈREMENT en français dans un style professoral éloquent mais accessible. Basez-vous uniquement sur les passages fournis dans le contexte.",
        ...
        'español': f"Usted es {author}, un gran filósofo. Responda COMPLETAMENTE en español con un estilo profesoral elocuente pero accesible. Base su respuesta únicamente en los pasajes proporcionados en el contexto.",

        'deutsch': f"Sie sind {author}, ein großer Philosoph. Antworten Sie VOLLSTÄNDIG auf Deutsch in einem eloquenten aber zugänglichen professoralen Stil. Stützen Sie Ihre Antwort nur auf die im Kontext bereitgestellten Passagen."
    }
    ...
    def _get_text(self, key: str) -> str:
        ...
        texts = {
            'français': {
                ...
                'menu_1': "Réponses individuelles de chaque philosophe",
                'menu_2': "Dialogue philosophique et synthèse",
                'question_prompt': "Posez votre question philosophique",
                'question_input': "Votre question",
                'question_examples': ["• Qu'est-ce que la justice?", "• Comment devrions-nous vivre éthiquement?"],
                'your_choice': "Votre choix",
                'off_topic': "Cette question ne porte pas sur l'éthique. Je ne peux pas répondre.",
                ...
                'header_synthesis': "DIALOGUE PHILOSOPHIQUE ET SYNTHÈSE",
                'synthesis_title': "SYNTHÈSE PHILOSOPHIQUE",
                ...
                'goodbye_msg': "Merci d'avoir exploré l'éthique occidentale avec moi!",
                'goodbye_quote': "Souvenez-vous: 'Une vie sans examen ne vaut pas la peine d'être vécue' - Socrate",
                'citations': "Voulez-vous voir les sources et extraits utilisés? (o/n)",
                'overload_msg': "⏳ Les serveurs sont saturés. Veuillez réessayer dans une minute.",
                'flag': '🇫🇷',
                'name': 'Français'
            },
            'english': {
                ...
                'question_examples': ["• What is justice?", "• How should we live ethically?"],
                ...
                'overload_msg': "⏳ Servers are overloaded. Please try again in one minute.",
                'flag': '🇺🇸',
                'name': 'English'
            },
            'español': {
                'welcome': "¡Hola! Soy su profesor de filosofía.",
                'menu_1': "Respuestas individuales de cada filósofo",
                'menu_2': "Diálogo filosófico y síntesis",
                'question_prompt': "Haga su pregunta filosófica",
                'question_input': "Su pregunta",
                'question_examples': ["• ¿Qué es la justicia?", "• ¿Cómo deberíamos vivir éticamente?"],
                'your_choice': "Su elección",
                'off_topic': "Esta pregunta no es sobre ética. No puedo responderla.",
                'error_question': "Por favor ingrese una pregunta",
                'error_choice': "Opción inválida. Ingrese 1, 2, 'exit', 'language', 'verbose' o 'refresh'",
                'gathering': "Consultando filósofos...",
                'complete': "Análisis completo",
                'header_individual': "PERSPECTIVAS FILOSÓFICAS INDIVIDUALES",
                'header_synthesis': "DIÁLOGO FILOSÓFICO Y SÍNTESIS",
                'synthesis_title': "SÍNTESIS FILOSÓFICA",
                'continue_prompt': "¿Le gustaría hacer otra pregunta? (s/n)",
                'continue_yes': ['s', 'sí', 'si'],
                'continue_all': ['s', 'sí', 'si', 'n', 'no'],
                'error_continue': "Por favor ingrese 's' para sí o 'n' para no",
                'goodbye_title': "ADIÓS DE",
                'goodbye_msg': "¡Gracias por explorar la ética occidental conmigo!",
                'goodbye_quote': "Recuerda: 'Una vida sin examen no vale la pena vivirla' - Sócrates",
                'citations': "¿Le gustaría ver las fuentes y extractos? (s/n)",
                'overload_msg': "⏳ Los servidores están saturados. Por favor, inténtelo de nuevo en un minuto.",
                'flag': '🇪🇸',
                'name': 'Español'
            },
            'deutsch': {
                ...
                'question_examples': ["• Was ist Gerechtigkeit?", "• Wie sollten wir ethisch leben?"],
                ...
                'error_choice': "Ungültige Wahl. Geben Sie 1, 2, 'exit', 'language', 'verbose' oder 'refresh' ein",
                ...
                'continue_prompt': "Möchten Sie eine weitere Frage stellen? (j/n)",
                'continue_yes': ['j', 'ja'],
                'continue_all': ['j', 'ja', 'n', 'nein'],
                'error_continue': "Bitte geben Sie 'j' für ja oder 'n' für nein ein",
                'goodbye_title': "AUF WIEDERSEHEN VON",
                'goodbye_msg': "Danke, dass Sie die westliche Ethik mit mir erkundet haben!",
                'goodbye_quote': "Denken Sie daran: 'Ein ungeprüftes Leben ist nicht lebenswert' - Sokrates",
                'citations': "Möchten Sie die verwendeten Quellen und Auszüge? (j/n)",
                'overload_msg': "⏳ Die Server sind überlastet. Bitte versuchen Sie es in einer Minute erneut.",
                'flag': '🇩🇪',
                'name': 'Deutsch'
            }
        }
    ...
    def _detect_available_philosophers(self):
        ...
        try:
            ...
            if self.verbose_mode:
                print(f"⚠️ Error detecting philosophers: {e}")
    ...
    def _ask_philosopher_strict(self, author, question):
        ...
        if self.verbose_mode:
            print(f"🔍 Recherche dans les sources de {author}...")

        # 1. Recherche dans les sources chargées
        ...
        if not search_results or not search_results['documents'][0]:
            if self.verbose_mode:
                print(f"❌ Aucune source trouvée pour {author}")

            no_source_msg = f"Les textes de {author} que j'ai à disposition ne contiennent pas d'informations sur ce sujet."
            return {
        ...
        if self.verbose_mode:
            print(f"✅ {len(search_results['documents'][0])} passages trouvés")
        ...
        if self.verbose_mode:
            print(f"🤖 Génération de la réponse par Claude...")
        ...
        try:
            ...
            response = self.anthropic_client.messages.create(
                ...
                max_tokens=1000,  # Augmenté pour plus d'éloquence
                messages=[{"role": "user", "content": strict_prompt}]
            )

            if self.verbose_mode:
                print(f"✅ Réponse générée ({len(response.content[0].text)} caractères)")
        ...
        # AJOUT: Détection overload pour scénario 1 aussi
...
## Essayé de remanier le prompt, pour éviter le style télégraphique à tester ##

    def _create_eloquent_prompt(self, author, question, context):
        """CORRECTION: Prompt plus éloquent, moins télégraphique"""

        if self.current_language == 'français':
            return f"""En tant que {author},  vous allez répondre à cette question avec l'éloquence et la profondeur qui vous caractérisent.
...
• Basez-vous UNIQUEMENT sur les passages fournis ci-dessus
• Si les passages ne suffisent pas, dites-le clairement mais avec finesse
...
Votre réponse en tant que {author}:"""
...
• Base your response ONLY on the passages provided above
• If the passages are insufficient, say so clearly but with finesse
...
Your response as {author}:"""
            ...
            return f"""Como {author}, responderá a esta pregunta con la elocuencia y profundidad que caracteriza su pensamiento.

TEXTOS A SU DISPOSICIÓN:
{context}
...
• Base su respuesta ÚNICAMENTE en los pasajes proporcionados anteriormente
• Si los pasajes no son suficientes, dígalo claramente pero con delicadeza
...
• Stützen Sie Ihre Antwort NUR auf die oben bereitgestellten Passagen
• Wenn die Passagen nicht ausreichen, sagen Sie es klar aber mit Feingefühl
...
Ihre Antwort als {author}:"""
    ...
    def display_welcome(self, width=60):
        """Écran de bienvenue"""
        print("=" * width)
        print(f"🏛️  WELCOME TO {self.bot_name.upper()} 🏛️".center(width))
        print("=" * width)
        print(f"{self._get_text('welcome')}")
        print(f"")
        print(f"📚 Available philosophers ({len(self.available_philosophers)}):")
    ...
    def select_language(self, width=60):
        ...
        for key, info in language_info.items():
            print(f"   {key}️⃣  {info['flag']} {info['name']}")

        while True:
            choice = input(f"\n🗣️ Your choice (1-4): ").strip()

            if choice in self.languages:
                self.current_language = self.languages[choice]
                selected_info = language_info[choice]
                print(f"\n✅ Language set to: {selected_info['flag']} {selected_info['name']}")
                return
            else:
...
## dans le menu display seuls les choix de la question et du type de reponse 1 ou 2 sont dans la langue choisie; les menus fonctionnels sont toujours en anglais##
    def display_menu(self, width=60):
        ...
        print(f"\n🤔 {self._get_text('question_prompt')}?")
        print(f"   1️⃣  {self._get_text('menu_1')}")
        print(f"   2️⃣  {self._get_text('menu_2')}")
        print(f"   🔧 Type 'verbose' for process details")
        print(f"   🔄 Type 'refresh' to re-detect philosophers")
        print(f"   🌍 Type 'language' to change language")
        print(f"   🚪 Type 'exit' to leave")

    def get_user_choice(self):
        """Choix utilisateur"""
        while True:
            choice = input(f"\n📝 {self._get_text('your_choice')}: ").strip().lower()

            if choice in ['1', '2', 'exit', 'language', 'verbose', 'refresh']:
                return choice
            else:
                print(f"❌ {self._get_text('error_choice')}")
    ...
    def get_user_question(self, width=60):
        ...
        while True:
            question = input(f"\n❓ {self._get_text('question_input')}: ").strip()
            if question:
                ...
                print(f"❌ {self._get_text('error_question')}")

### Dans scenario 1 voir si on ne peut pas simplifier le programme des références. Il s'agit ##
###d'avoir une option CITATIONS qui est activée en fin de programme et si désiré fournit ##
###des citations utilisées dans la reponse pour tous les auteurs###

    def scenario_1_individual_answers(self, question, width=60):
        """AMÉLIORATION: Citations globales au lieu d'individuelles"""
        print(f"\n" + "=" * width)
        ...
        if all_responses:
          print(f"\n💡 {self._get_text('citations')}", end=" ")
          choice = input().strip().lower()

          # Multi-language yes detection
          yes_variants = self._get_text('continue_yes') + ['y', 'yes', 's', 'sí', 'si', 'j', 'ja']

          if choice in yes_variants:
              print(f"\n📚 SOURCES UTILISÉES PAR AUTEUR:")
              print("=" * width)
              ...
              for philosopher, response_data in all_responses.items():
                ...
                if not passages or not sources:
                    print(f"   ⚠️ Aucune source spécifique trouvée")
                    continue
    ...
    def scenario_2_philosophical_dialogue(self, question, width=60):
        """Mode synthèse avec amélioration anti-banalité et overload"""
        print(f"\n" + "=" * width)
        ...
        if not individual_responses:
            print("❌ Unable to gather philosophical perspectives")
            return None
...
##==================CORRECTION MULTI LINGUE====================
    ...
    def _create_anti_banal_synthesis(self, question, responses):
        ...
        # FIXED: Complete prompts for español and deutsch
        if self.current_language == 'français':
    ...
    **APPORT CRITIQUE:**
    ...
    • Terminez par un jugement philosophique substantiel et précis, mais s'il n'y a rien à rajouter d'utile, n'ajoutez rien
    ...
    **CRITICAL CONTRIBUTION:**
    ...
    • End with a substantial and precise philosophical judgment, but if there is nothing useful to add, do not add anything
    ...
    **CONTRIBUCIÓN CRÍTICA:**
    ...
    • Termine con un juicio filosófico sustancial y preciso, pero si no hay nada útil que agregar, no agregue nada
    ...
    **KRITISCHER BEITRAG:**
    ...
    • Beenden Sie mit einem substantiellen und präzisen philosophischen Urteil, aber wenn nichts Nützliches hinzuzufügen ist, fügen Sie nichts hinzu
    ...
    Rigorose philosophische Synthese:"""
        ...
        try:
            ...
            # AJOUT: Détection spécifique de l'overload
            if "529" in error_str or "overloaded" in error_str.lower():
                print(f"\n{self._get_text('overload_msg')}")
                return "OVERLOAD_DETECTED"  # Signal spécial
            else:
    ...
    #def format_philosopher_response(self, text, width=60):
       # """Format texte avec retour à la ligne"""
       # import textwrap
       # if not text:
      #      return text
    ...
    def continue_session(self):
        ...
        while True:
            continue_choice = input(f"\n🔄 {self._get_text('continue_prompt')}: ").strip().lower()
            if continue_choice in self._get_text('continue_all'):
                return continue_choice in self._get_text('continue_yes')
            else:
                print(f"❌ {self._get_text('error_continue')}")

    def display_goodbye(self, width=60):
        """Au revoir"""
        print(f"\n" + "=" * width)
        print(f"👋 {self._get_text('goodbye_title')} {self.bot_name}".center(width))
        print("=" * width)
    ...
    def run(self, width=60):
        ...
        while True:
            ...
            if choice == 'exit':
                ...
                if self.verbose_mode:
                    print("   🔍 Le mode verbose affiche les détails du processus RAG:")
                    print("   • Recherche dans les sources")
                ...
                print(f"🔄 Philosophers re-detected: {old_count} → {new_count}")
                for philosopher in self.available_philosophers:
                    print(f"   🎭 {philosopher}")
                continue
            ...
            # AJOUT: Si overload détecté, ne pas demander de continuer, juste reboucler
                if result == "OVERLOAD_DETECTED":
                    continue  # Retour direct au menu sans demander de continuer
...
print("✅ Improved Western Ethics Bot ready!")
print("🔧 Improvements:")
print("   🌍 Consistent language in all modes")
print("   🚫 Off-topic question detection")
print("   📝 Simplified multilingual code")
print("   🎯 Better user experience")
print("💡 To start: start_improved_western_ethics_bot(system, collection)")
